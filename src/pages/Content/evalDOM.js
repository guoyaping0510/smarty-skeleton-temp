export default function evalDOM() {
  ...
  const classProps = {
    position: 'fixed',
    zIndex: 999,
    background: agrs.background||"#f4f4f4"
  }
  if (agrs.animation) {
  ...
  function drawBlock({width, height, top, left, zIndex = 999, background = "#f4f4f4", radius, subClas} = {}) {
  ...
  DrawPageframe.prototype = {
    ...
    withHeader: function() {
      if (agrs.header) {
        ...
        const hBackground = background || agrs.background||"#f4f4f4";
        if (hHeight) {
          drawBlock({
