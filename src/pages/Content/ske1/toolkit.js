 ...
 * @returns
 */
export function mergePosition(nodes) {
  const graph = {};
  ...
  function findEnd(gNodeTail, isX, size) {
 ...
 * @returns
 */
export function mergeHighOverlapNodes(nodes) {
  // 标记节点是否合并过
  const mergedFlags = Array.from({ length: nodes.length }, () => false);
  // 合并后的节点
  const mergedNodes = [];
 ...
 * @return {*}
 */
export function weightNode(nodes) {
  nodes.forEach((node1) => {
    const { x, y, w, h } = node1;
